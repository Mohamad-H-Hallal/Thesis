...
| Backups | Automatic 35-day expiry; deletion ledger prevents restoration to active identity; no destructive backup rewrite | Prove OCI lifecycle and isolated restore behavior. |
...
| Hosting/subprocessors/transfers | OCI Jeddah E4 is selected; tenancy setup is incomplete | Executed provider terms/DPA, account owner, exact region, support-access/transfer facts, private storage, backup, Firebase, ArcGIS, SMTP, and GEE entries. |
